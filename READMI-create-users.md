...
`update_password`: — Может приниматьз начения `always` и `on_create`, по-умолчанию используется `on_create`.

> В случае использования значения `always` без указания пароля, пароль будет удалён, а при следующем логине пользователю будет предложено задать новый.

`groups`: Список групп, к которым принадлежит пользователь через запятую.

`comment`: Комментаций к пользователю.
...
`ssh_key`: Позволяет задать ключ для доступа к сервера по SSH.
